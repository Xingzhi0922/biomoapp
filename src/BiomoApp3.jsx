...
const HomeScreen = ({ setPage }) => {
  // 5 metric cards: all navigate to reports with specific metric tab
  const metrics = [
    { label:"Air Quality",  val:"612", unit:"ppm", color:C.green,    pill:"green",  pillText:"Good",       arc:[67,27], arcBg:"#EAF3DE", metricKey:"co2"    },
    { label:"Room Use",     val:"8",   unit:"/13", color:C.blue,     pill:"blue",   pillText:"8 busy",     arc:[44,50], arcBg:"#E6F1FB", metricKey:"occ"    },
    { label:"Electricity",  val:"3.8", unit:"kWh", color:"#BA7517",  pill:"amber",  pillText:"+12% today", arc:[58,36], arcBg:"#FAEEDA", metricKey:"energy" },
    { label:"Noise",        val:"58",  unit:"dB",  color:"#7C5CBF",  pill:"purple", pillText:"Moderate",   arc:[50,44], arcBg:"#EEEDFE", metricKey:"noise"  },
    { label:"Water",        val:"42",  unit:"L/d", color:C.green,    pill:"teal",   pillText:"On track",   arc:[35,59], arcBg:"#E1F5EE", metricKey:"water"  },
  ];
  return (
    <ScreenBody>
      {/* header */}
      <div style={{ display:"flex", justifyContent:"space-between",
        ...
        <div onClick={()=>setPage({id:"alerts",metric:null})} style={{ width:36, height:36, borderRadius:"50%",
          background:C.bgCard, border:`0.5px solid ${C.border}`,
          display:"flex", alignItems:"center", justifyContent:"center",
      ...
      {/* ticker — 5 items, equal width, no scroll */}
      <div style={{ background:"#2C2C2A", borderRadius:14, padding:"8px 10px",
        display:"flex", justifyContent:"space-between", marginBottom:12 }}>
        {[["Air","612","ppm","Good","#5DCAA5"],
          ["Rooms","8/13","busy","↓−1","#5DCAA5"],
          ["Noise","58","dB","Mod.","#C4B0F0"],
          ["Energy","3.8","kWh","▲+12%","#F0997B"],
          ["Water","42","L","▼−8%","#5DCAA5"]
        ].map(([l,val,unit,change,col])=>(
          <div key={l} style={{ textAlign:"center", flex:1 }}>
            <div style={{ fontSize:8, color:"rgba(255,255,255,0.38)", textTransform:"uppercase",
              letterSpacing:"0.06em", marginBottom:1 }}>{l}</div>
            <div style={{ fontSize:11, color:"#fff", fontWeight:600, lineHeight:1.2 }}>
              {val}<span style={{ fontSize:8, color:"rgba(255,255,255,0.45)", marginLeft:1 }}>{unit}</span>
            </div>
            <div style={{ fontSize:9, color:col, lineHeight:1.1 }}>{change}</div>
          </div>
        ))}
      </div>
      {/* metric cards — 3×2 grid, compact */}
      <div style={{ fontSize:10, color:C.textSub, marginBottom:6 }}>Tap a card for details</div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:7, marginBottom:12 }}>
        {metrics.map(m=>(
          <div key={m.label} onClick={()=>setPage({id:"reports", metric:m.metricKey})}
            style={{ background:C.bgCard, borderRadius:14, border:`0.5px solid ${C.border}`,
              padding:"10px 8px", cursor:m.to?"pointer":"default" }}>
            <div style={{ fontSize:8.5, color:C.textSub, fontWeight:500, textTransform:"uppercase",
              letterSpacing:"0.05em", marginBottom:5 }}>{m.label}</div>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:5 }}>
              <div style={{ display:"flex", alignItems:"flex-end" }}>
                <span style={{ fontSize:20, fontWeight:600, lineHeight:1, color:m.color }}>{m.val}</span>
                <span style={{ fontSize:9, color:C.textSub, marginBottom:1, marginLeft:2 }}>{m.unit}</span>
              </div>
            ...
            <Pill color={m.pill} style={{ fontSize:9, padding:"2px 7px" }}>{m.pillText}</Pill>
          </div>
        ))}
      </div>
      {/* active alerts */}
      ...
      {ALERTS.filter(a=>a.sev==="critical"||a.sev==="warning").map(a=>(
        <div key={a.id} onClick={()=>setPage({id:"alerts",metric:null})}
          style={{ background:C.bgCard, borderRadius:14, border:`0.5px solid ${C.border}`,
...
//  REPORTS SCREEN
// ─────────────────────────────────────────────
const ReportsScreen = ({ initMetric=null }) => {
  const [metric, setMetric] = useState(initMetric||"energy");
  ...
  const CFG = {
    ...
    water: {
      label:"Water", unit:"L/day", color:C.blue,
      score:85, scoreLabel:"vs target",
      monthData:{ cur:[48,45,52,46,42,38,40], prev:[55,52,58,53,48,44,46] },
      weekData:{ cur:[44,46,52,40,38,34,30], prev:[50,52,58,46,44,40,36] },
      thisWeekTotal:"42 L/day", vsLast:"−8%", vsLastBad:false, monthAvg:"44 L/day",
      insight:"Water usage down 8% vs last week — on track. Kitchen and Kafis account for 60% of total. No leaks detected.",
      insightBg:"#E6F1FB", insightColor:"#042C53",
      goals:[
        { label:"Daily target ≤ 50 L",       pct:85, color:C.blue,  tc:"#185FA5" },
        { label:"Month vs last month",        pct:90, color:C.green, tc:"#27500A" },
        { label:"Leak detection — 0 events",  pct:100,color:C.green, tc:"#27500A" },
      ],
    },
  };
  const mc = CFG[metric];
  ...
  const WATER_DAILY = { thisWeek:[44,46,52,40,38,34,30], lastWeek:[50,52,58,46,44,40,36] };
  const [cArr, pArr] = metric==="energy"
    ...
    : metric==="water"
    ? [WATER_DAILY.thisWeek, WATER_DAILY.lastWeek]
  ...
  return (
    <ScreenBody>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:5, marginBottom:16 }}>
        {[
          { key:"energy", label:"Energy",    score:61, color:C.amber,   sub:"vs target" },
          { key:"co2",    label:"Air",       score:74, color:C.green,   sub:"rooms OK"  },
          { key:"occ",    label:"Rooms",     score:62, color:C.blue,    sub:"busy"      },
          { key:"noise",  label:"Noise",     score:78, color:"#7C5CBF", sub:"dB OK"     },
          { key:"water",  label:"Water",     score:85, color:C.blue,    sub:"on track"  },
        ].map(m=>(
          <div key={m.key} onClick={()=>setMetric(m.key)}
            ...
            <ScoreRing pct={m.score} color={m.color} size={40}/>
      ...
      <div style={{ display:"flex", background:"#F1EFE8", borderRadius:9,
        ...
        {[["energy","Energy"],["co2","Air"],["occ","Rooms"],["noise","Noise"],["water","Water"]].map(([k,lbl])=>(
      ...
      <Card style={{ marginBottom:10 }}>
        {DAYS.map((d,i)=>{
          ...
          const suffix = metric==="energy"?" kWh":metric==="co2"?" ppm":metric==="noise"?" dB":metric==="water"?" L":"%";
          return (
...
export default function BiomoApp() {
  const [page, setPage] = useState({id:"home", metric:null});
  return (
    <div style={{ display:"flex", justifyContent:"center", alignItems:"flex-start",
      ...
      <div style={{ width:340, background:C.bg, borderRadius:42,
        ...
        <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
          {page.id==="home"     && <HomeScreen    setPage={setPage}/>}
          {page.id==="floors"   && <FloorsScreen/>}
          {page.id==="reports"  && <ReportsScreen initMetric={page.metric}/>}
          {page.id==="alerts"   && <AlertsScreen/>}
          {page.id==="settings" && <SettingsScreen/>}
        </div>
        <BottomNav active={page.id} setPage={(id)=>setPage({id,metric:null})}/>
      </div>
    </div>
  );
}